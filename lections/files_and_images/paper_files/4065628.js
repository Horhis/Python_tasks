...
jQuery(document).ready(function(){
    jQuery('.pmc-labs-ad-close').bind('click', closePmcLabsAd);
});

function closePmcLabsAd(e){ 
	e.stopPropagation();
	e.preventDefault();
		
	closePmcLabsAdSetCookie();
	 	
	jQuery('#pmc-labs-ad').hide();
}


function closePmcLabsAdSetCookie(){
    var d = new Date();
    d.setTime(d.getTime() + (7*24*60*60*1000)); // 7 days
    var expires = "expires="+d.toUTCString();
    
    var newCookie = "hide_pmc_labs_ad=true;domain=.ncbi.nlm.nih.gov;path=/;" + expires;
